...
public class RobotLocalizationViewer2 {
	...
	public synchronized void initLocViewer( ){		
		...
		updateViewer( start[0], start[1], start[2], -1, -1, loc.getCurrentProbDist());
		transMat = null;
		senseMat = null;
		initFlag = true;
	}
	...
	public synchronized void updateOneStep( ){		
		if( initFlag) {
			int[] tXYH = new int[3]; 
			int[] sXY = new int[2]; 
			boolean sR = loc.update( tXYH, sXY);
			if( sR)
				updateViewer(  tXYH[0], tXYH[1], tXYH[2], sXY[0], sXY[1], loc.getCurrentProbDist());	
			else
				updateViewer(  tXYH[0], tXYH[1], tXYH[2], -1, -1, loc.getCurrentProbDist());	
	...
	public void updateViewer( int tX, int tY, int tH, int sX, int sY, double[] currProb) {
		int x, y, h;
		double posProbMax, help;
		...
		for( x=0; x<rows; x++) {
			for( y=0; y<cols; y++) {
				...
				if( currProb != null)
					for( h=0; h<head; h++)
						help += currProb[x*cols*head + y*head + h];
	...
	public void updateTransitionView() {
		...
		for( x=0; x<rows; x++) {
			for( y=0; y<cols; y++) {
				for( h=0; h<head; h++) {
					//states[x][y][h].setText( String.format("%.2f", transMat.getTij( tXCount*cols*head + tYCount*head + tHCount, 
					//															+	x*cols*head + y*head + h)));
					states[x][y][h].setText( String.format("%.2f", transMat.getTxyhToXYH( tXCount, tYCount, tHCount, x, y, h)));
	...
	public void updateSensorView() {
		...
		if( senseMat == null)
			senseMat = loc.getObservationMatrices();
		...
		for( x=0; x<rows; x++) {
			for( y=0; y<cols; y++) {
				...
				for( h=0; h<4; h++) {
					states[x][y][h].setBackground(Color.white);		
					s = String.format("%.3f", senseMat.getOrXY( sXCount, sYCount, x, y));
